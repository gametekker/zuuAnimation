...
public:
    ...
    */
    LuminanceColormap(const std::vector<std::array<double, 3>>& colors, int shades_for_each_color = 16);

    void generateColormap(const std::vector<std::array<double, 3>>& colors);
     ...
     * @return std::array<double, 3> The RGB color.
     */
    std::array<double, 3> getColor(int colorIdx, int shadeIdx) const;
    ...
    std::vector<std::vector<std::array<double, 3>>> getColormap() const;
